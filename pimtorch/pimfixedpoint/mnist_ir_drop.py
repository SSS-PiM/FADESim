...
from fixedPoint import ir_drop_em_fastsolve
...
class FcMnist(nn.Module):
    def __init__(self):
        ...
        self.fc1 = fpnn.Linear_sp(784, 32)
        self.fc2 = fpnn.Linear_sp(32, 10)
...
class LeNetMnist(nn.Module):
    def __init__(self):
        super().__init__()
        self.lenet = nn.Sequential(fpnn.Conv2d_sp(1, 6, (5, 5), padding=(2, 2)),
                                   nn.Tanh(),
                                   nn.MaxPool2d(kernel_size=(2, 2), stride=2),
                                   fpnn.Conv2d_sp(6, 16, (5, 5), padding=(2, 2)),
                                   nn.Tanh(),
                                   nn.MaxPool2d(kernel_size=(2, 2), stride=2),
                                   nn.Flatten(),
                                   nn.Dropout(p=0.2),
...
def main():
    ...
    parser.add_argument('--net', type=int, default=0, metavar='NET',
                        help='use which model (0:fc_mnst 1:fc5mnist 2:conv_mnist 3:lenet_mnist -1 :none)')
...
if __name__ == '__main__':
    ...
    # input = torch.rand([1, 64], device="cuda").to(torch.float32)
    # w = torch.rand([64, 64], device="cuda").to(torch.float32)
    # out2 = torch.matmul(input, w)
    
    # T1 = time.time()
    # out1 = fpnn.splitArr_matmul_nn(input, w, 8, 1, 8, 8, 8, fpnn.commonConst.phyArrParams.defaultArrMode, torch.float32)
    # T2 = time.time()
